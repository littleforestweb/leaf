...
class ServerSideTable(object):
    ...
    def _custom_filter(self, data):
        ...
        for i in range(len(self.columns)):
            column_name = self.columns[i]["column_name"]
            searchTerm = self.request_values.get("sSearch_" + str(i), "")
            if searchTerm != "":
                data = [row for row in data if re.compile("(?i)" + self.request_values["sSearch_" + str(i)]).search(str(row[column_name]))]
