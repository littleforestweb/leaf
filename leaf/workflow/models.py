                ...
                for list_item_url_path in list_items_url_path:
                    ...
                    if thisType != 8:
                        for srv in Config.DEPLOYMENTS_SERVERS:
                            ...
                            original_content = data
                            original_content_changed = data.replace(Config.LEAFCMS_SERVER, urljoin(Config.PREVIEW_SERVER, Config.DYNAMIC_PATH, '/leaf/'))
                            current_app.logger.debug(srv["webserver_url"] + Config.DYNAMIC_PATH.strip('/') + '/leaf/')
                            current_app.logger.debug(Config.LEAFCMS_SERVER)
                            data = data.replace(Config.LEAFCMS_SERVER, urljoin(srv["webserver_url"], Config.DYNAMIC_PATH, '/leaf/'))
                            with open(local_path, "w") as outFile:
                                outFile.write(data)
