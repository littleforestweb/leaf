    ...
    if not listName and thisType == 1:
        ...
        if os.path.exists(local_path):
            for srv in Config.DEPLOYMENTS_SERVERS:
                ...
                data = data.replace(str(os.path.join(Config.PREVIEW_SERVER.rstrip("/"), srv["webserver_url"])))
                data = data.replace(str(os.path.join(Config.LEAFCMS_SERVER, srv["webserver_url"])))
