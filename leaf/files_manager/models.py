...
def list_img_files(site_id, archive):
    """
    Retrieve img files from the specified site.
    ...
    try:
        ...
        if site_id == False:
            ...
            if archive != "1":
                ...
                FROM site_assets 
                LEFT JOIN user ON site_assets.modified_by = user.id 
                WHERE site_id IN ({sites_result_placeholder}) AND (site_assets.mimeType = 'image/jpeg' OR site_assets.mimeType = 'image/png' OR site_assets.mimeType = 'image/gif' OR site_assets.mimeType = 'image/bmp') AND site_assets.status <> -1
                """
            else:
                files_query = f"""
                SELECT site_assets.id, site_assets.path, site_assets.filename, site_assets.mimeType, 
            ...
            if archive != "1":
                files_query = f"SELECT site_assets.id, site_assets.path, site_assets.filename, site_assets.mimeType, IFNULL({userUsernameEmail}, '{first_user_admin[0]}, {first_user_admin[1]}, {first_user_admin[2]}') AS modified_by, site_assets.created FROM site_assets LEFT JOIN user ON site_assets.modified_by = user.id WHERE site_id = %s AND (site_assets.mimeType = 'image/jpeg' OR site_assets.mimeType = 'image/png' OR site_assets.mimeType = 'image/gif' OR site_assets.mimeType = 'image/bmp') AND site_assets.status <> -1"
            else:
                files_query = f"SELECT site_assets.id, site_assets.path, site_assets.filename, site_assets.mimeType, IFNULL({userUsernameEmail}, '{first_user_admin[0]}, {first_user_admin[1]}, {first_user_admin[2]}') AS modified_by, site_assets.created FROM site_assets LEFT JOIN user ON site_assets.modified_by = user.id WHERE site_id = %s AND (site_assets.mimeType = 'image/jpeg' OR site_assets.mimeType = 'image/png' OR site_assets.mimeType = 'image/gif' OR site_assets.mimeType = 'image/bmp) AND site_assets.status = -1"
            mycursor.execute(files_query, [site_id])
        ...
        if folder_paths:
            # Filter files based on user access
            if session["is_admin"] == 0:
                access_files = [{"id": file[0], "Path": os.path.join("/", file[1]), "Filename": file[2], "Mime Type": file[3], "Created By": file[4], "Created": file[5]} for file in img_files if any(file[1].startswith(path.lstrip("/")) for path in folder_paths)]
            else:
                access_files = [{"id": file[0], "Path": os.path.join("/", file[1]), "Filename": file[2], "Mime Type": file[3], "Created By": file[4], "Created": file[5]} for file in img_files]
        else:
            access_files = [{"id": file[0], "Path": os.path.join("/", file[1]), "Filename": file[2], "Mime Type": file[3], "Created By": file[4], "Created": file[5]} for file in img_files]
...
def is_rss_feed(file_path):
    ...
    try:
        with open(full_file_path, 'r', encoding='utf-8') as file:
            ...
            return content.startswith('<?xml') and '<rss' in content
    except Exception as e:
