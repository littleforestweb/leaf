...
window.addEventListener('DOMContentLoaded', async function main() {
    ...
    let json = await $.get("/api/get_task_requests", function (result) {
        return result;
    });

    // Set dataset
    let dataset = [];
    json = json["task_requests"];
    ...
    $('#table').DataTable({
        dom: 'Brtip',
        buttons: {
            buttons: [{text: 'Export', extend: 'csv', filename: 'Tasks Report', className: 'btn-export'}],
            dom: {
                button: {
                    className: 'btn'
                }
