...
async function populateEditDynamicListDialog(accountId, reference, type, itemToSelect = false) {
...
async function publishDynamicList(accountId, reference, env, preview_server, dynamic_path, thisTemplate, thisParameters, fieldsToLink, justPreview, lastEntry, thisCountry = false, thisButton) {
    ...
    if (!thisButton.classList.value.includes('disabled')) {
    ...
    $.ajax({
        ...
        data: JSON.stringify({"country_to_update": thisCountry}),
        ...
        success: function (updated) {
            ...
            if ((env !== 'saveOnly' && env !== 'save')) {
                if (thisTemplate !== '') {
                    openInNewTab(preview_server + dynamic_path + thisTemplate + '.html?' + thisParameters + '=' + fieldsToLink + (lastEntry ? lastEntry : ''));
                } else {
...
async function updateDynamicList(accountId, reference, env, preview_server, dynamic_path, justPreview, thisButton) {
            ...
            success: function (updated) {
                ...
                if (env === 'preview' && !justPreview || env === 'save' || env === 'saveOnly') {
                    if (thisCountry && thisCountry !== '' && thisCountry.length > 0) {
                        publishDynamicList(accountId, reference, env, preview_server, dynamic_path, thisTemplate, thisParameters, fieldsToLink, false, false, thisCountry, thisButton);
                    } else {
                        publishDynamicList(accountId, reference, env, preview_server, dynamic_path, thisTemplate, thisParameters, fieldsToLink, false, false, false, thisButton);
                    }
                } else {
...
async function addDynamicList(accountId, reference, env, preview_server, dynamic_path, justPreview, thisButton) {
    ...
    if (!form_data[0] || (form_data[0] && form_data[0].mandatoryFields != true)) {
        ...
        $.ajax({
            ...
            success: function (updated) {
                ...
                if (env === 'preview' && !justPreview || env === 'save' || env === 'saveOnly') {
                    if (thisCountry && thisCountry !== '' && thisCountry.length > 0) {
                        publishDynamicList(accountId, reference, env, preview_server, dynamic_path, thisTemplate, thisParameters, fieldsToLink, false, updated.lastEntry, thisCountry, thisButton);
                    } else {
                        publishDynamicList(accountId, reference, env, preview_server, dynamic_path, thisTemplate, thisParameters, fieldsToLink, false, updated.lastEntry, false, thisButton);
                    }
                } else {
...
async function deleteDynamicListEntries(accountId, reference, env, preview_server, dynamic_path, thisButton) {
